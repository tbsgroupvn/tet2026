...
// Cleanup rate limit map periodically (on each request, cheap check)
...
app.post('/api/game-results', async (req, res) => {
  try {
    ...
    await db.execute({
      ...
      args: [playerId, game, coinsWon, details],
    });
...
app.post('/api/rewards', async (req, res) => {
  try {
    ...
    const playerCheck = await db.execute({ sql: 'SELECT total_coins FROM players WHERE id = ?', args: [playerId] });
    if (!playerCheck.rows[0] || playerCheck.rows[0].total_coins < coinCost) {
    ...
    const result = await db.execute({
      ...
      args: [playerId, rewardName, coinCost, paymentMethod, paymentInfo],
    });
    if (totalCoins !== undefined) {
      await db.execute({
        sql: "UPDATE players SET total_coins = ?, updated_at = datetime('now','localtime') WHERE id = ?",
